...
TSharedPtr<FJsonObject> FBlueprintCommands::HandleCreateBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!ParentClass.IsEmpty())
	{
		...
		// Try project module (custom C++ classes)
		if (!FoundClass)
		{
			FString ProjectName = FApp::GetProjectName();
			FString ProjectClassPath = FString::Printf(TEXT("/Script/%s.%s"), *ProjectName, *ClassName);
		...
		if (!FoundClass)
		{
			FString EngineClassPath = FString::Printf(TEXT("/Script/Engine.%s"), *ClassName);
		...
		if (!FoundClass)
		{
			FString AIModulePath = FString::Printf(TEXT("/Script/AIModule.%s"), *ClassName);
			FoundClass = LoadClass<AActor>(nullptr, *AIModulePath);
		}
...
TSharedPtr<FJsonObject> FBlueprintCommands::HandleConnectBlueprintNodes(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Trigger ReconstructNode for structural pin types
	FName PinCategory = TargetPin->PinType.PinCategory;
	bool bShouldReconstruct = (
		...
		TargetPinName == TEXT("Class") ||
		TargetPinName == TEXT("Template")
	);
			...
			for (const TPair<FString, TSharedPtr<FJsonValue>>& Param : (*ParamsObj)->Values)
			{
				...
				if (ParamPin)
				{
					...
					else if (ParamValue->Type == EJson::Array)
					{
						...
						{
							if (ParamPin->PinType.PinCategory == UEdGraphSchema_K2::PC_Struct &&
...
TSharedPtr<FJsonObject> FBlueprintCommands::HandleCreateGameplayEffect(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (UEditorAssetLibrary::DoesAssetExist(AssetPath + EffectName))
	{
		return FCommonUtils::CreateErrorResponse(FString::Printf(TEXT("GameplayEffect already exists: %s"), *EffectName));
	}
...
TSharedPtr<FJsonObject> FBlueprintCommands::HandleSetPinDefaultValue(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Get the value and set it based on pin type
	FString ValueStr;
	double ValueNum = 0;
	...
	else if (Params->HasField(TEXT("value")))
	{
		...
		if (Params->TryGetArrayField(TEXT("value"), ArrayValue))
		{
			if (ArrayValue->Num() == 3)
			{
				...
				float Y = (*ArrayValue)[1]->AsNumber();
				float Z = (*ArrayValue)[2]->AsNumber();
				TargetPin->DefaultValue = FString::Printf(TEXT("(X=%f,Y=%f,Z=%f)"), X, Y, Z);
			}
			else if (ArrayValue->Num() == 4)
			{
				// Could be Rotator (Pitch, Yaw, Roll) + extra or Color (R, G, B, A)
...
TSharedPtr<FJsonObject> FBlueprintCommands::HandleAddGenericNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Special handling for SpawnActor - replace with stable CallFunction
	if (NodeClassName.Contains(TEXT("SpawnActor")))
	{
		UFunction* SpawnFunc = UGameplayStatics::StaticClass()->FindFunctionByName(TEXT("BeginDeferredActorSpawnFromClass"));
		if (!SpawnFunc)
		{
			return FCommonUtils::CreateErrorResponse(TEXT("Failed to find BeginDeferredActorSpawnFromClass function"));
		}
		...
		// Set ActorClass if provided
		FString ActorClassPath;
		if (Params->TryGetStringField(TEXT("ActorClass"), ActorClassPath) ||
		    Params->TryGetStringField(TEXT("actor_class"), ActorClassPath))
		{
			UClass* ActorClass = LoadClass<AActor>(nullptr, *ActorClassPath);
			if (ActorClass)
			{
				UEdGraphPin* ClassPin = CallNode->FindPin(TEXT("ActorClass"));
				if (ClassPin)
				{
		...
		NodeResult->SetStringField(TEXT("node_id"), NewNode->GetName());
		NodeResult->SetStringField(TEXT("node_class"), TEXT("K2Node_CallFunction"));
		NodeResult->SetStringField(TEXT("note"), TEXT("SpawnActorFromClass replaced with BeginDeferredActorSpawnFromClass CallFunction for stability"));
		ResultObj->SetObjectField(TEXT("result"), NodeResult);
		return ResultObj;
	}

	NewNode = FCommonUtils::CreateNodeByClassName(TargetGraph, NodeClassName, NodePosition);
	if (!NewNode)
	{
